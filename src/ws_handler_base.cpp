...
namespace webservice{
	...
	namespace{
		...
		std::map< ws_server_session* > map_intersection(
			ws_sessions::map const& map1,
			std::map< ws_identifier > const& map2
		){
			auto first1 = map1.begin();
			auto last1 = map1.end();
			auto first2 = map2.begin();
			auto last2 = map2.end();

			std::map< ws_server_session* > result;
			auto d_first = std::inserter(result, result.begin());

			while(first1 != last1 && first2 != last2){
				if(map1.key_comp()(*first1, *first2)){
					++first1;
				}else{
					if(!(map1.key_comp()(*first2, *first1))){
						*d_first++ = (*first1++).get();
					}
					++first2;
				}
	...
	void ws_handler_base::send_text(
		...
		list_->async_call(
			[identifier, buffer = std::move(buffer)](
				ws_sessions::map const& sessions
			)mutable{
	...
	void ws_handler_base::send_text(
		std::map< ws_identifier > const& identifiers,
		shared_const_buffer buffer
	){
		list_->async_call(
			[identifiers, buffer = std::move(buffer)](
				...
				auto valid_sessions = map_intersection(sessions, identifiers);
	...
	void ws_handler_base::close(
		...
		list_->async_call(
			[identifier, reason](ws_sessions::map const& sessions){
				if(sessions.count(identifier) == 0){
					return;
				}
	...
	void ws_handler_base::close(
		...
		list_->async_call(
			[identifiers, reason](ws_sessions::map const& sessions){
				auto valid_sessions = map_intersection(sessions, identifiers);
				for(auto session: valid_sessions){
					session->send(
						boost::beast::websocket::close_reason(reason));
	...
	void ws_handler_base::close(boost::beast::string_view reason){
		list_->async_call(
			[reason](ws_sessions::map const& sessions){
				for(auto& session: sessions){
					session->send(
						boost::beast::websocket::close_reason(reason));
				}
	...
	void ws_handler_base::map_server(class server& server){
